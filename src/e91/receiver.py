...
class Receiver(Participant):
  ...
  def _calculate_measurements(self):
    ...
    self.measurements['b1'] = QuantumCircuit(self.qr, self.cr, name='measureB1')
    self.measurements['b1'].s(self.qr[1])
    self.measurements['b1'].h(self.qr[1])
    self.measurements['b1'].t(self.qr[1])
    self.measurements['b1'].h(self.qr[1])
    self.measurements['b1'].measure(self.qr[1], self.cr[1])

    # Measure the spin projection of Bob's qubit onto the b_2 direction (standard Z basis)
    self.measurements['b2'] = QuantumCircuit(self.qr, self.cr, name='measureB2')
    self.measurements['b2'].measure(self.qr[1], self.cr[1])

    # Measure the spin projection of Bob's qubit onto the b_3 direction (V basis)
    self.measurements['b3'] = QuantumCircuit(self.qr, self.cr, name='measureB3')
    self.measurements['b3'].s(self.qr[1])
    self.measurements['b3'].h(self.qr[1])
    self.measurements['b3'].tdg(self.qr[1])
    self.measurements['b3'].h(self.qr[1])
    self.measurements['b3'].measure(self.qr[1], self.cr[1])
